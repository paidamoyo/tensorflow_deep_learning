...
def pz1_given_z2y(FLAGS, y_logits, z_latent_rep):
    ...
    w_h2_mu, b_h2_mu = create_h_weights('h2_mu', 'decoder', [2 * FLAGS['encoder_h_dim'], FLAGS['encoder_h_dim']])

    w_mu_z1, w_var_z1, b_mu_z1, b_var_z1 = create_z_weights('z_1_decoder',
                                                            [FLAGS['decoder_h_dim'], FLAGS['latent_dim']])
    # Model
    # Decoder hidden layer
    h1_z = activated_neuron(z_latent_rep, w_h1_z, b_h1_z)
    h2_z = activated_neuron(h1_z, w_h2_z, b_h2_z)

    h1_y = activated_neuron(y_logits, w_h1_y, b_h1_y)
