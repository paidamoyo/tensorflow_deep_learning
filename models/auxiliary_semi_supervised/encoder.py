...
def q_z_given_yx(y, x, latent_dim, num_classes, hidden_dim, input_dim, reuse=False):
    with tf.variable_scope("encoder_M2", reuse=reuse):
        # Variables
        w_h1, b_h1 = create_nn_weights('h1_z', 'encoder', [input_dim + num_classes, hidden_dim])
        ...
        # Hidden layers
        h1 = mlp_neuron(tf.concat([y, x], axis=1), w_h1, b_h1)
