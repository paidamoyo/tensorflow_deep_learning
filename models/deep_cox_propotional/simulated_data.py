...
class SimulatedData(object):
    def __init__(self, hr_ratio=5.0,
    ...
    def _gaussian_H(self, x, c=0.0, rad=0.5):
        """
    ...
    def generate_data(self, N, method='gaussian', gaussian_config={}, **kwargs):
        """
        ...
        print("p_death:{}".format(p_death.shape))
        ...
        print("risk:{}, mean risk:{}".format(risk.shape, np.mean(risk).shape))
        risk -= np.mean(risk)
        ...
        for i in range(N):
            if self.treatment_group and data[i, -1] == 0:
                ...
                # TODO check if this this a bug
                death_time[i] = np.random.exponential(p_death[i]) / exp(risk[i])
        print("death time:{}".format(death_time.shape))

        # Censor anything that is past end time
        print("end_time:{}, censored:{}".format(self.end_time, len(np.where(death_time > self.end_time))))
...
if __name__ == '__main__':
    sim = SimulatedData()
    data = sim.generate_data(method='linear', N=5000)
    print(data)
